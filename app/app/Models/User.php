...
class User extends Authenticatable
{
    ...
    public function addBalance($date, $amount, $account_id){
        $account   = BankAccount::where('created_by', '=', $this->creatorId())->where('id', '=', $account_id)->get()->first();

        if($account->current_balance == 0){
            $account->current_balance = $account->opening_balance + $amount;
        } else {
        ...
        $year      = $date->year;
        $month     = $date->month;
        $entryDate = $year . '-' . ($month < 10 ? '0'.$month : $month) . '-' . '01';

        $balances  = \DB::table('balance')->where('created_by', '=', $this->creatorId())->where('account_id', '=', $account->id)->get();
        
        if($balances->count() == 0){
            \DB::insert('insert into balance (`date`, `previous_month`, `amount`, `created_by`, `account_id`) values (?, ?, ?, ?, ?)', [$entryDate, $account->opening_balance, $amount, $this->creatorId(), $account->id]);
            return;
        }
        
        $balances  = $balances->sortBy('date');
        
        if($balances->contains('date', '=', $entryDate)){
            $status = 'searching'; 
            $prevMonth = 0;
            foreach($balances as $balance){
                $balance = (array)$balance;
                if($balance['date'] != $entryDate && $status == 'searching'){
                    continue;
                } else if ($balance['date'] == $entryDate){
                    $balance['amount'] += $amount;
                    $prevMonth = $balance['previous_month'] + $balance['amount'];
                    \DB::update('update balance set `amount` = ? where `id` = ?', [$balance['amount'], $balance['id']]);
                    $status = 'updating';
                } else if ($status == 'updating') {
                    $balance['previous_month'] = $prevMonth;
                    $prevMonth += $balance['amount'];
                    \DB::update('update balance set previous_month = ? where id = ?', [$balance['previous_month'], $balance['id']]);
                } else {
                    break;
                }
            }
        } else {
            $lastItem  = (array) $balances->sortByDesc('date')->first();
            $firstItem = (array) $balances->sortBy('date')->first();
            
            $dateEntry = Carbon::createFromFormat('Y-m-d', $entryDate);
            $firstDate = Carbon::createFromFormat('Y-m-d', $firstItem['date']);
            $lastDate  = Carbon::createFromFormat('Y-m-d', $lastItem['date']);
            
            if($dateEntry->lt($firstDate)){
                \DB::insert('insert into balance (`date`, `previous_month`, `amount`, `created_by`, `account_id`) values (?, ?, ?, ?, ?)', [$entryDate, $account->opening_balance, $amount, $this->creatorId(), $account->id]);
                $prevMonth = $account->opening_balance + $amount;
                foreach($balances as $balance){
                    $balance = (array)$balance;
                    $balance['previous_month'] = $prevMonth;
                    $prevMonth += $balance['amount'];
            ...
            } else if ($dateEntry->gt($lastDate)){
                $prevMonth = $lastItem['previous_month'] + $lastItem['amount'];
                \DB::insert('insert into balance (`date`, `previous_month`, `amount`, `created_by`, `account_id`) values (?, ?, ?, ?, ?)', [$entryDate, $prevMonth, $amount, $this->creatorId(), $account->id]);
            } else {
                $refBalances  = \DB::table('balance')->where('created_by', '=', $this->creatorId())->where('account_id', '=', $account->id)->get();
                $balances->prepend(array('date' => $entryDate, 'previous_month' => 0, 'amount' => $amount, 'created_by' => $this->creatorId()));
                $balances = $balances->sortBy('date');
                $prevMonth = 0;
                foreach($balances as $balance){
                    $balance = (array)$balance;
                    if($balance['previous_month'] == $account->opening_balance){
                        $prevMonth = $balance['previous_month'] + $balance['amount'];
                        continue;
                    }
                    $balance['previous_month'] = $prevMonth;
                    $prevMonth = $prevMonth + $balance['amount'];

                    if($refBalances->contains('date', $balance['date'])){
                        \DB::update('update balance set previous_month = ? where id = ?', [$balance['previous_month'], $balance['id']]);
                    } else {
                        \DB::insert(
                            'insert into balance (`date`, `previous_month`, `amount`, `created_by`, `account_id`) values (?, ?, ?, ?, ?)', 
                            [
                                $balance['date'], 
                                $balance['previous_month'], 
                                $balance['amount'], 
                                $this->creatorId(), 
                                $account->id]
                        );
        ...
        $this->cleanBalance($account->id);
    }

    public function cleanBalance($account_id){
        $balances  = \DB::table('balance')->where('created_by', '=', \Auth::user()->creatorId())->where('account_id', '=', $account_id)->get();
        if($balances->count() != 0){
            $balances = $balances->sortBy('date');
            $previous_month = 0;
            $status = 'scanning';
            foreach($balances as $balance){
                $balance = (array) $balance;
                if($balance['amount'] == 0){
                    \DB::delete('DELETE FROM balance WHERE id = ?', [$balance['id']]);
                    if($status != 'moving'){
                        $previous_month = $balance['previous_month'];
                        $status = 'moving';
                    }
                } else if($balance['amount'] != 0 && $status == 'moving'){
                    $balance['previous_month'] = $previous_month;
                    \DB::update('UPDATE balance set previous_month = ? where id = ?', [$balance['previous_month'], $balance['id']]);
                    $status = 'scanning';
                }
            }
