...
<div class="wizard-form collapse">
    <div class="d-grid gap-2 mb-3">
        <a href="{{ route('samples.import', 'product') }}" class="btn btn-primary btn-icon icon-left">
            <span class="btn-inner--icon"><i class="fas fa-download"></i></span>
            <span class="btn-inner--text"> {{__('Sample')}}</span>
        </a>
    </div>
    {{ Form::open(['route' => 'productservice.import.headings', 'class' => 'dropzone import-dropzone', 'enctype' => 'multipart/form-data']) }}
    {{ Form::close() }}
</div>
{{ Form::open(['method' => 'PUT', 'route' => 'productservice.import.store', 'class' => 'wizard-form collapse pt-3']) }}
    <p class="h4">{{ __('Please select the appropriate column names') }}</p>
    {{ Form::hidden('path', '') }}
    <div class="row">
        <div class="col-md-6 pb-2">
            {{ Form::label('name', __('Product Name')) }}
            {{ Form::select('name', ['' => '---'], null, ['class' => 'form-control selectric second-form','required'=>'required']) }}
        </div>
        <div class="col-md-6 pb-2">
            {{ Form::label('sku', __('Product SKU')) }}
            {{ Form::select('sku', ['' => '---'], null, ['class' => 'form-control selectric second-form','required'=>'required']) }}
        </div>
        <div class="col-md-6 pb-2">
            {{ Form::label('quantity', __('Quantity')) }}
            {{ Form::select('quantity', ['' => '---'], null, ['class' => 'form-control selectric second-form', 'data-is-required']) }}
        </div>
        <div class="col-md-6 pb-2">
            {{ Form::label('unit', __('Unit')) }}
            {{ Form::select('unit', ['' => '---'], null, ['class' => 'form-control selectric second-form', 'required'=>'required']) }}
        </div>
        <div class="col-md-4 pb-2">
            {{ Form::label('sale_price', __('Sale Price')) }}
            {{ Form::select('sale_price', ['' => '---'], null, ['class' => 'form-control selectric second-form','required'=>'required']) }}
        </div>
        <div class="col-md-4 pb-2">
            {{ Form::label('purchase_price', __('Purchase Price')) }}
            {{ Form::select('purchase_price', ['' => '---'], null, ['class' => 'form-control selectric second-form','required'=>'required']) }}
        </div>
        <div class="col-md-4 pb-2">
            {{ Form::label('tax', __('Tax')) }}
            {{ Form::select('tax', ['' => '---'], null, ['class' => 'form-control selectric second-form', 'data-is-required']) }}
        </div>
        <div class="col-md-6 pb-2">
            {{ Form::label('category', __('Category')) }}
            {{ Form::select('category', ['' => '---'], null, ['class' => 'form-control selectric second-form', 'data-is-required']) }}
        </div>
        <div class="col-md-6 pb-2">
            {{ Form::label('type', __('Type')) }}
            {{ Form::select('type', ['' => '---'], null, ['class' => 'form-control selectric second-form', 'data-is-required']) }}
        </div>
...
<div class="wizard-form collapse">
    <div class="text-center pt-4 pb-5">
        <i class="fas fa-circle-check fa-9x text-success success-indicator"></i>
        <p class="import-message">{{ __('Data imported successfully') }}</p>
    </div>
</div>
<script>
